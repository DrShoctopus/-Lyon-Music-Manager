...
# Register your app at https://www.last.fm/api/account/create. Packaged builds
# must not embed a repo-wide Last.fm shared secret; users can enter their own
# API credentials in Settings. Environment variables remain useful for source
# runs and automated tests.
_ENV_LASTFM_API_KEY: str = (os.environ.get("LYON_LASTFM_API_KEY") or "").strip()
_ENV_LASTFM_API_SECRET: str = (os.environ.get("LYON_LASTFM_API_SECRET") or "").strip()
...
def _lastfm_credentials(settings: "Settings | None" = None) -> tuple[str, str]:
    if settings is not None:
        key = str(getattr(settings, "lastfm_api_key", "") or "").strip()
        secret = str(getattr(settings, "lastfm_api_secret", "") or "").strip()
        if key or secret:
            return key, secret
    return _ENV_LASTFM_API_KEY, _ENV_LASTFM_API_SECRET


def _lastfm_sign(params: dict[str, str], api_secret: str | None = None) -> str:
    keys = sorted(k for k in params if k not in ("format", "api_sig"))
    raw = "".join(f"{k}{params[k]}" for k in keys) + (api_secret if api_secret is not None else _ENV_LASTFM_API_SECRET)
    return hashlib.md5(raw.encode("utf-8")).hexdigest()


def _lastfm_post(params: dict[str, str], api_secret: str | None = None) -> dict:
    params["format"] = "json"
...
def lastfm_api_configured(settings: "Settings | None" = None) -> bool:
    key, secret = _lastfm_credentials(settings)
    return bool(key and secret)


def lastfm_auth_url(token: str, settings: "Settings | None" = None) -> str:
    key, _secret = _lastfm_credentials(settings)
    return f"{_LASTFM_AUTH_URL}?api_key={key}&token={token}"
...
class ScrobblerService(QObject):
    ...
    def lastfm_auth_url(self, token: str) -> str:
        return lastfm_auth_url(token, self._settings)
    ...
    def _submit_now_playing(self, track: "Track") -> None:
        if (
            ...
            api_key, api_secret = _lastfm_credentials(self._settings)
        else:
            api_key = api_secret = ""
        if api_key and api_secret:
            ...
            QThreadPool.globalInstance().start(_HttpTask(lambda p=params, s=api_secret: _lastfm_post(p, s)))
    ...
    def start_lastfm_auth(self) -> None:
        ...
        def get_token() -> None:
            result = _lastfm_post({"method": "auth.getToken", "api_key": api_key}, api_secret)
            if "error" in result:
    ...
    def poll_lastfm_session(self) -> None:
        ...
        def do_poll() -> None:
            api_key, api_secret = _lastfm_credentials(self._settings)
            if not api_key or not api_secret:
                self._session_failed.emit("Last.fm API key/secret not configured.")
                return
            result = _lastfm_post({
